...
echo "=== 1d. Dry run install (--docker / --no-docker accepted) ==="
"${TMP}/runner.sh" install --dry-run -y --no-bootstrap-ssh --no-docker >/dev/null
"${TMP}/runner.sh" install --dry-run -y --no-bootstrap-ssh --docker >/dev/null
